class DiplomaAutoclicker {
    ...
    addWordManually() {
        ...
        // Проверяем, завершён ли текущий диплом
    ...
    checkDiplomaCompletion() {
        ...
        if (this.charCount >= currentDiplomaData.charsNeeded && !this.diplomaCompleted) {
            if (this.currentDiploma < 6) {
                // Переходим к следующему диплому
                this.currentDiploma++;
                this.currentText = '';
                this.charCount = 0;
    ...
    checkModeUnlocks() {
        ...
        if (this.totalClicks < 100) {
            normalProgress.textContent = this.totalClicks + '/100';
        }
        
        if (this.totalClicks < 500) {
            panicProgress.textContent = this.totalClicks + '/500';
        }
        
        if (this.totalClicks >= 100 && btnNormal.disabled) {
        ...
        if (this.totalClicks >= 500 && btnPanic.disabled) {
    ...
    updateModeButtons() {
        ...
        if (this.totalClicks < 100) {
            ...
            btnNormal.title = 'Откроется после 100 кликов';
        }
        ...
        if (this.totalClicks < 500) {
            ...
            btnPanic.title = 'Откроется после 500 кликов';
        }
    }
    ...
    setMode(mode) {
        if (mode === 'normal' && this.totalClicks < 100) {
            showNotification('🔒 Нужно 100 кликов для открытия!');
            return;
        }
        if (mode === 'panic' && this.totalClicks < 500) {
            showNotification('🔒 Нужно 500 кликов для открытия!');
            return;
        }
    ...
    reset() {
        ...
        document.getElementById('btn-normal').innerHTML = '🔒 Нормально<br><small><span id="normal-progress">0/100</span></small>';
        ...
        document.getElementById('btn-panic').innerHTML = '🔒 Паника<br><small><span id="panic-progress">0/500</span></small>';
